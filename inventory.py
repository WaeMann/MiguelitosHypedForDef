...
class InventoryPage(QWidget):
    def __init__(self, switch_callback=None, role="admin", username=""):
        super().__init__()
    ...
    def initUI(self):
        ...
        reports_btn.setFixedSize(170, 36)
        ...
        ingredients_btn.setFixedSize(170, 36)
        ...
        self.table.setHorizontalHeaderLabels(
            ["#", "Product Name", "Price", "Qty Left", "Category", "Image Path"]
        )
        ...
        if self.role != "admin":
            # Hide Delete and Add Item buttons
            self.delete_btn.hide()
            self.add_btn.hide()
            # Make price and quantity read-only in the edit form
            self.ef_price.setReadOnly(True)
            self.ef_price.setStyleSheet(
                self.ef_price.styleSheet() +
                "QLineEdit { background: #f0ede0; color: #888; }"
            )
            self.ef_qty.setReadOnly(True)
            self.ef_qty.setStyleSheet(
